...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        "*** END YOUR CODE HERE ***"
        '''
        allPossible = EI_HOOK(self).observe(observation, emissionModel, pacmanPosition)
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        self.beliefs = EI_HOOK(self).elapseTime(gameState)
        #util.raiseNotDefined()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        PF_HOOK(self).initializeUniformly()
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        PF_HOOK(self).observe(gameState, observation, emissionModel, pacmanPosition)
        #util.raiseNotDefined()
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        PF_HOOK(self).elapseTime(gameState)
        #util.raiseNotDefined()
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return PF_HOOK(self).getBeliefDistribution()
        #util.raiseNotDefined()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        JPF_HOOK(self).initializeUniformly()
    ...
    def observeState(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        JPF_HOOK(self).observeState(noisyDistances, emissionModels, pacmanPosition)
    ...
    def elapseTime(self, gameState):
        ...
        '''
        JPF_HOOK(self).elapseTime(gameState)

    def getBeliefDistribution(self):
        "*** YOUR CODE HERE ***"
        return JPF_HOOK(self).getBeliefDistribution()
        #util.raiseNotDefined()
...
#######################################################
###MY CODE BEGINS HERE###
class EI_HOOK(object):
    def __init__(self, ei):
        self.ei = ei
        
    def observe(self, observation, emissionModel, pacmanPosition):
        if observation is None:
            allPossible = util.Counter()
            allPossible[self.ei.getJailPosition()] = 1.0
        else:
            allPossible = self.ei.beliefs
            for p in self.ei.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
    ...
    def elapseTime(self, gameState):
        allPossible = util.Counter()
        for oldPos in self.ei.legalPositions:
            newPosDist = self.ei.getPositionDistribution(self.ei.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                allPossible[newPos] += self.ei.beliefs[oldPos] * prob
        allPossible.normalize()
        return allPossible


class PF_HOOK(object):
    def __init__(self, pf):
        self.pf = pf
        
    def evenSampleList(self):
        return [self.pf.legalPositions[i % len(self.pf.legalPositions)]
                               for i in xrange(self.pf.numParticles)]
    def evenWeightList(self):
        return [1.0] * self.pf.numParticles
        
    def initializeUniformly(self):
        self.pf.sample_list = self.evenSampleList()
        self.pf.weight_list = self.evenWeightList()

    def observe(self, gameState, observation, emissionModel, pacmanPosition):
        if observation is None:
            self.pf.sample_list = [self.pf.getJailPosition()] * self.pf.numParticles
        else:
            self.pf.weight_list = [v * emissionModel[util.manhattanDistance(p, pacmanPosition)] 
                                   for (p, v) in zip(self.pf.sample_list, self.pf.weight_list)]
            self.pf.sample_list = self.evenSampleList() if sum(self.pf.weight_list) == 0.0 else \
                                  util.nSample(self.pf.weight_list, self.pf.sample_list, self.pf.numParticles)
        self.pf.weight_list = self.evenWeightList()
            
    def elapseTime(self, gameState):
        self.pf.sample_list = [util.sample(self.pf.getPositionDistribution(self.pf.setGhostPosition(gameState, oldPos)))
                               for oldPos in self.pf.sample_list]
        
    def getBeliefDistribution(self):
        allPossible = util.Counter()
        for (p, v) in zip(self.pf.sample_list, self.pf.weight_list): allPossible[p] += v
        allPossible.normalize()
        return allPossible


class JPF_HOOK(PF_HOOK):
    def __init__(self, pf):
        self.pf = pf
    
    def evenSampleList(self):
        legalPositionProducts = list(itertools.product(self.pf.legalPositions, repeat=self.pf.numGhosts))
        factor = max(self.pf.numParticles/len(legalPositionProducts) - 1, 0) #40
        num1 = len(legalPositionProducts)*(factor) #
        num2 = self.pf.numParticles - num1
        #print num1, num2, num1 + num2, self.pf.numParticles, len(self.pf.legalPositions), self.pf.numGhosts
        
        result = [legalPositionProducts[i % len(legalPositionProducts)] for i in xrange(num1)] \
               + [random.choice(legalPositionProducts) for _ in xrange(num2)]
        
        return result

    def checkedEvenSampleList(self, observations):
        result_list = self.evenSampleList()
        for ghostIndex in xrange(self.pf.numGhosts):
            if observations[ghostIndex] is None:
                result_list = [self.pf.getParticleWithGhostInJail(t, ghostIndex) for t in result_list]
        return result_list
        
    def observeState(self, observations, emissionModels, pacmanPosition):
        for ghostIndex in xrange(self.pf.numGhosts):
            if observations[ghostIndex] is None:
                self.pf.sample_list = [self.pf.getParticleWithGhostInJail(t, ghostIndex) for t in self.pf.sample_list]
            else:
                self.pf.weight_list = [v * emissionModels[ghostIndex][util.manhattanDistance(t[ghostIndex], pacmanPosition)]
                                       for (t, v) in zip(self.pf.sample_list, self.pf.weight_list)]
        self.pf.sample_list = self.checkedEvenSampleList(observations) if sum(self.pf.weight_list) == 0.0  else \
                                  util.nSample(self.pf.weight_list, self.pf.sample_list, self.pf.numParticles)
        self.pf.weight_list = self.evenWeightList()
   
    def elapseTime(self, gameState):
        self.pf.sample_list = [tuple([util.sample(getPositionDistributionForGhost(setGhostPositions(gameState, t), ghostIndex, self.pf.ghostAgents[ghostIndex]))
                                      for ghostIndex in xrange(self.pf.numGhosts)]) 
                               for t in self.pf.sample_list]
