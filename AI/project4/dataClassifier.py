...
def enhancedFeatureExtractorDigit(datum):
    ...
    ##
    """
    return EFE_HOOK().enhancedFeatureExtractorDigit(datum)
...
def basicFeatureExtractorPacman(state):
    ...
    for action in state.getLegalActions():
        ...
        featureCounter['capCount'] = len(successor.getCapsules())
        featureCounter['scoreCount'] = successor.getScore()
        featureCounter['suicideCount'] = sum([util.manhattanDistance(successor.getPacmanPosition(), p) for p in successor.getGhostPositions()])
...
def enhancedPacmanFeatures(state, action):
    ...
    """
    return EFE_HOOK().enhancedPacmanFeatures(state, action)
    features = util.Counter()
    "*** YOUR CODE HERE ***"
    util.raiseNotDefined()
    return features
...
def analysis(classifier, guesses, testLabels, testData, rawTestData, printImage):
    ...
    return EFE_HOOK().analysis(classifier, guesses, testLabels, testData, rawTestData, printImage)
    # Put any code here...
    # Example of use:
...
def runClassifier(args, options):
    ...
    for i in xrange(len(trainingData)):
        l = trainingLabels[i]
        e = 0 if l in (1, 2, 3, 5, 7) else 1 if l in (0, 4, 6, 9) else 2
        print e, l
        raw_input()
        trainingData[i][(e, 0, 0)] = l

    for i in xrange(len(validationData)):
        l = validationLabels[i]
        e = 0 if l in (1, 2, 3, 5, 7) else 1 if l in (0, 4, 6, 9) else 2
        validationData[i][(e, 0, 0)] = l
    ...
    #for i in xrange(len(rawTrainingData)):
    #    print rawTrainingData[i], trainingLabels[i]
    #    raw_input()
...
class EFE_HOOK(object):
    def getProb(self, datum, x, y):
        return (1 if datum.getPixel(x, y) > 0 else 0) \
              +(1 if datum.getPixel(x, y + 1) > 0 else 0) \
              +(1 if datum.getPixel(x + 1, y) > 0 else 0) \
              +(1 if datum.getPixel(x + 1, y + 1) > 0 else 0)
              
    def enhancedFeatureExtractorDigit(self, datum):
        #a = datum.getPixels()
        features = util.Counter()
        for x in range(DIGIT_DATUM_WIDTH - 1):
            for y in range(DIGIT_DATUM_HEIGHT - 1):
                #features[(x, y)] = 1 if datum.getPixel(x, y) > 0 else 0
                features[(x, y, x, y + 1, x + 1, y, x + 1, y + 1)] = self.getProb(datum, x, y)
        return features
        
    def enhancedPacmanFeatures(self, state, action):
        features = util.Counter()
        for action in state.getLegalActions():
            successor = state.generateSuccessor(0, action)
            foodCount = successor.getFood().count()
            featureCounter = util.Counter()
            #featureCounter['foodCount'] = foodCount
            #featureCounter['stop'] = 0
            #featureCounter['suicide'] = -1 * sum([util.manhattanDistance(successor.getPacmanPosition(), p) for p in successor.getGhostPositions()])
            #featureCounter['FoodAgent'] = featureCounter['foodCount'] * 50 + featureCounter['suicide']
            features[action] = featureCounter
        return features
    
    def analysis(self, classifier, guesses, testLabels, testData, rawTestData, printImage):
        pass
        #for #i in range(len(guesses)):
            #prediction = guesses[i]
            #truth = testLabels[i]
                ...
                #print rawTestData[i]
                #print testData[i]
                #printImage(testData[prediction])
                #printImage(testData[truth])
